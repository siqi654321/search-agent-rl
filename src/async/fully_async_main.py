...
class FullyAsyncTaskRunner:
    ...
    def _initialize_components(self, config) -> None:
        ...
        total_train_steps_ref = self.components["rollouter"].get_total_train_steps.remote()
        total_train_steps = ray.get(total_train_steps_ref)
        trainer_set_total_train_steps_ref = self.components["trainer"].set_total_train_steps.remote(total_train_steps)
        ray.get(trainer_set_total_train_steps_ref)

        # max_queue_size
        max_queue_size_ref = self.components["rollouter"].get_max_queue_size.remote()
        max_queue_size = ray.get(max_queue_size_ref)
        message_queue = MessageQueue.remote(config, max_queue_size)
        message_queue_client = MessageQueueClient(message_queue)
        self.components["message_queue"] = message_queue
        self.components["message_queue_client"] = message_queue_client

        rollouter_set_mq_ref = self.components["rollouter"].set_message_queue_client.remote(self.components["message_queue_client"])
        ray.get(rollouter_set_mq_ref)
        trainer_set_mq_ref = self.components["trainer"].set_message_queue_client.remote(self.components["message_queue_client"])
        ray.get(trainer_set_mq_ref)
        ...
        trainer_set_param_sync_ref = self.components["trainer"].set_parameter_synchronizer.remote(param_synchronizer)
        ray.get(trainer_set_param_sync_ref)
        ...
        trainer_load_ckpt_ref = self.components["trainer"].load_checkpoint.remote()
        param_version = ray.get(trainer_load_ckpt_ref)
        rollouter_load_ckpt_ref = self.components["rollouter"].load_checkpoint.remote()
        ray.get(rollouter_load_ckpt_ref)
        sync_weights_ref = param_synchronizer.sync_weights.remote(
            version=param_version,
            validate=val_before_train,
            use_trainer_do_validate=config.async_training.use_trainer_do_validate,
        )
        ray.get(sync_weights_ref)
        wait_last_valid_ref = param_synchronizer.wait_last_valid.remote()
        ray.get(wait_last_valid_ref)
