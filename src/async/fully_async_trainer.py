...
class FullyAsyncTrainer(SeparateRayPPOTrainer):
    ...
    def __init__(
        ...
        self.sync_aligned_grouping = config.async_training.get("sync_aligned_grouping", False)
        self.sync_aligned_group_size = self.required_samples * self.trigger_parameter_sync_step
        self.effective_required_samples = (
            self.sync_aligned_group_size if self.sync_aligned_grouping else self.required_samples
        )
        self.buffered_groups: dict[int, list[Any]] = {}
        self.next_expected_group_id = 0
        total_gpus = (
    ...
    def _get_samples_from_queue(self) -> tuple[None, None] | tuple[int, Any]:
        ...
        if self.sync_aligned_grouping:
            queue_samples = None
            while queue_samples is None:
                ready_group = self.buffered_groups.get(self.next_expected_group_id)
                if ready_group is not None and len(ready_group) >= self.sync_aligned_group_size:
                    queue_samples = self.buffered_groups.pop(self.next_expected_group_id)
                    self.next_expected_group_id += 1
                    break

                sample, queue_len = self.message_queue_client.get_sample_sync()
                if sample is None:
                    buffered_size = len(self.buffered_groups.get(self.next_expected_group_id, []))
                    break

                sample = ray.cloudpickle.loads(sample)
                group_id = sample.source_group_id
                group_samples = self.buffered_groups.setdefault(group_id, [])
                group_samples.append(sample)

                if group_id == self.next_expected_group_id and len(group_samples) % 64 == 0:

            if queue_samples is None:
                return None, None
        else:
            queue_samples = []
        ...
        if self.sync_aligned_grouping:
            queue_samples.sort(key=lambda sample: sample.source_prompt_index)
    ...
    def _fit_collect_metrics(self, batch):
        ...
        self.metrics_aggregator.add_step_metrics(
            metrics=self.metrics, sample_count=self.effective_required_samples, timestamp=time.time()
        )
        self._log_validation_data()
    ...
    def _fit_save_checkpoint(self):
        ...
        if self.config.trainer.save_freq > 0 and self.current_param_version > 0 and (
            ...
            metrics.update(
                {
                    ...
                    "fully_async/static/sync_aligned_grouping": float(self.sync_aligned_grouping),
                    "fully_async/static/effective_required_samples": self.effective_required_samples,
                }
            )
    ...
    async def _trigger_parameter_sync_after_step(self, validate: bool = False):
        ...
        """
        if not self.sync_aligned_grouping and self.local_trigger_step < self.trigger_parameter_sync_step and not validate:
            self.local_trigger_step += 1
            return
