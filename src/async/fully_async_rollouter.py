...
class FullyAsyncRollouter(SeparateRayPPOTrainer):
    ...
    def __init__(
        ...
        self.sync_aligned_group_size = self.required_samples * self.config.async_training.trigger_parameter_sync_step
        self.configured_max_concurrent_samples = config.async_training.get("max_concurrent_samples", None)
    ...
    async def set_max_required_samples(self):
        async with self.lock:
            ...
            default_max_concurrent_samples = len(self.async_rollout_manager.server_handles) * 16
            if self.configured_max_concurrent_samples is None:
                self.max_concurrent_samples = default_max_concurrent_samples
            else:
                self.max_concurrent_samples = int(self.configured_max_concurrent_samples)
    ...
    async def _feed_samples(self):
        ...
        for epoch, batch_dict in continuous_iterator:
            ...
            source_group_id = source_prompt_index // self.sync_aligned_group_size
        ...
        async with self.lock:
            ...
            try:
                await asyncio.wait_for(
                    self.async_rollout_manager.clear_kv_cache(), timeout=CLEAR_KV_CACHE_TIMEOUT_S
                )
