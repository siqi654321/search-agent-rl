    ...
    async def generate_sequences_no_post(
        ...
        try:
            ...
            for i in range(len(batch)):
                ...
                tasks.append(
                    asyncio.create_task(self._partial_run_agent_loop(dict(sampling_params), trajectory_info[i], **kwargs))
                )
...
class FullyAsyncAgentLoopManager(AgentLoopManager):
    ...
    async def clear_kv_cache(self):
        results = await asyncio.gather(
            *[replica.clear_kv_cache() for replica in self.rollout_replicas], return_exceptions=True
        )
