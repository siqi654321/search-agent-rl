...
class ParameterSynchronizer:
    ...
    def __init__(self, config, trainer, rollouter, mq):
        ...
        self.mq_client = mq
        actor_wg_ref = trainer.get_actor_wg.remote()
        self.actor_wg = ray.get(actor_wg_ref)
        rollout_wg_ref = rollouter.get_rollout_wg.remote()
        self.rollout_wg = ray.get(rollout_wg_ref)
        ...
        replicas_ref = rollouter.get_replicas.remote()
        replicas = ray.get(replicas_ref)
    ...
    def sync_weights(self, version, validate=False, global_steps=0, use_trainer_do_validate=False):
        ...
        pause_ref = self.rollouter.pause.remote()
        ray.get(pause_ref)
