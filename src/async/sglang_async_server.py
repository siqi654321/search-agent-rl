...
class FullyAsyncSGLangReplica(SGLangReplica):
    ...
    async def cancel(self):
        ...
        refs = [server.cancel.remote() for server in self.servers]
        await asyncio.gather(*refs)

    async def resume(self):
        """Resume each rollout server."""
        refs = [server.resume.remote() for server in self.servers]
        await asyncio.gather(*refs)

    async def clear_kv_cache(self):
        refs = [server.clear_kv_cache.remote() for server in self.servers]
