...
class QwenToolParser(ToolParser):                                                 
    def __init__(self, tokenizer) -> None:                                            
        super().__init__(tokenizer)                             
        self._tool_call_regex = regex.compile(r"<tool_call>(.*?)</tool_call>", regex.DOTALL)                                                                                                                                             
        self._query_regex = regex.compile(r"<query>(.*?)</query>", regex.DOTALL)                                          
    @rollout_trace_op                                                             
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:                                                                                                                                      
        loop = get_event_loop()                                             
        text = await loop.run_in_executor(None, self.tokenizer.decode, responses_ids)                                         
        ...
        if "<tool_call>" in text and "</tool_call>" in text:              
            raw_queries = self._tool_call_regex.findall(text)        
            content = self._tool_call_regex.sub("", text)          
        elif "<query>" in text and "</query>" in text:         
            raw_queries = self._query_regex.findall(text)          
            content = self._query_regex.sub("", text)              
        else:                                                  
            return text, []                                        
        ...
        for q in raw_queries:                                  
            ...
            if not query:                                          
                continue                                               
            args = {"query_list": [query]}                         
            function_calls.append(FunctionCall(name="search", arguments=json.dumps(args, ensure_ascii=False)))                                  
        return content, function_calls 
