...
class TaskRunner:
    ...
    def add_teacher_policy_worker(self, config, teacher_policy_cls):
        """Add a forward-only teacher policy worker for on-policy distillation."""
        from verl.trainer.ppo.ray_trainer import Role
        ...
        if config.get("on_policy_distill", {}).get("enable", False):
    ...
    def run(self, config):
        ...
        # Add a separate teacher policy worker for Thinking Machines-style OPD.
        self.add_teacher_policy_worker(config, actor_rollout_cls)

        # validate config. The OPD teacher reuses ref-style log-prob forward but
        # is not part of the normal KL reference-policy path, so include it for
        # ref micro-batch validation only.
        validate_config(
            config=config,
            use_reference_policy=need_reference_policy(config)
            or config.get("on_policy_distill", {}).get("enable", False),
