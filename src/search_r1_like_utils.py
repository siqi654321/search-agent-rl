...
def _passages2string(retrieval_result):
    ...
    # Compatibility mode: if the retrieval server already returns a natural-language
    # summary in `document.contents` (e.g. our summarize-compat endpoint), return it
    # directly to avoid wrapping with "Doc N" headers.
    try:
        if isinstance(retrieval_result, list) and retrieval_result:
            first = retrieval_result[0]
            if isinstance(first, dict):
                doc = first.get("document") if isinstance(first.get("document"), dict) else first
                contents = doc.get("contents") if isinstance(doc, dict) else None
                if isinstance(contents, str):
                    c = contents.strip()
                    if c.startswith("Your query is:") and "summarized" in c:
                        summaries: list[str] = []
                        for item in retrieval_result:
                            if not isinstance(item, dict):
                                continue
                            d = item.get("document") if isinstance(item.get("document"), dict) else item
                            cc = d.get("contents") if isinstance(d, dict) else None
                            if isinstance(cc, str) and cc.strip():
                                summaries.append(cc.strip().split('\n')[0])
